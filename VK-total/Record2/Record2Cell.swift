...
class Record2Cell: UITableViewCell {
    ...
    func configureCell(record: Record, profiles: [RecordProfiles], groups: [RecordGroups], likes: [Likes], indexPath: IndexPath, tableView: UITableView, cell: UITableViewCell, viewController: UIViewController) -> CGFloat {
        ...
        votersLabels.removeAll(keepingCapacity: false)
    ...
    func configurePoll(_ poll: Poll, topY: CGFloat) -> CGFloat {
        ...
        let view = UIView()
        view.tag = 100
        //if !poll.color.isEmpty { view.backgroundColor = UIColor(hexaRGB: poll.color) }
    ...
    func getRowHeight(record: Record) -> CGFloat {
        ...
        for index in 0...9 {
            if record.mediaType[index] == "poll" {
                if let poll = record.poll {
                    let qLabelSize = getPollLabelSize(text: poll.question, font: qLabelFont)
                    var viewY: CGFloat = qLabelSize.height + 70
                    
                    for ind in 0...poll.answers.count-1 {
                        let aLabelSize = getPollAnswerLabelSize(text: poll.answers[ind].text, font: aLabelFont)
...
class PollAnswerLabel: UILabel {

    override func draw(_ rect: CGRect) {
        let inset = UIEdgeInsets(top: 0, left: 10, bottom: 0, right: 50)
        super.drawText(in: rect.inset(by: inset))
    }
}

class PollQuestionLabel: UILabel {

    override func draw(_ rect: CGRect) {
        let inset = UIEdgeInsets(top: 0, left: 60, bottom: 0, right: 5)
        super.drawText(in: rect.inset(by: inset))
    }
}
